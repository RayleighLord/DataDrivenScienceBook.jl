...
<div class="nb-intro">
	<div class="header-box"> 
		...
		<p class="nb-intro-title"> Linear Regression </p>
...
# ╔═╡ 49d79a91-b9bc-4df6-ae13-521dd0242d1c
begin 
	x = 3 # True slope
	a = collect(-2:0.25:2)
	b = x .* a .+ randn(size(a))
...
# ╔═╡ 490e6a3e-a514-4444-b76d-1ce2e3b55f59
begin
	U, Σ, V = svd(a)
	x̃ = V * (diagm(Σ) \ (U' * b))

	# Another way to compute it is using the pseudoinverse pinv command
	# A′ = pinv(a)
	# x̃ = A′ * b
end

# ╔═╡ 889e89ce-ec0b-4abc-a949-68dba858342a
begin

	scatter(a, b; marker = (:cross, 8), color = :red, label = L"\mathrm{Noisy}", legend = :topleft, xlabel = L"a", ylabel = L"b", size= (600, 500))
	plot!(a, x * a, color = :black, label = L"\mathrm{True \ Line}")
	plot!(a, x̃ .* a, line = :dash, color = :blue, label = L"\mathrm{Regression \ Line}")
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "1e52c661fa62964cd52b9e63a79af2926b4620e6"
